...
-- b. Retrieve the product names and quantities for a specific order (order_id = 1)
SELECT p.product_name, oi.quantity
FROM order_items oi
JOIN products p ON oi.product_id = p.id
WHERE oi.order_id = 1;

-- c. Retrieve all orders placed by a specific customer (customer_id = 1)
...
-- a. Reduce stock quantities after a customer places an order (order_id = 1)
UPDATE products p
...
-- a. Remove an order and its associated order items (order_id = 1)
